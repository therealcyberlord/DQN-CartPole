...
replay_buffer = Replay.ReplayMemory(replay_memory_size)
...
def update_step(policy_net, target_net, replay_buffer, gamma, optimizer, criterion, batch_size):
    ...
    non_final_next_states = torch.tensor(np.array([s[2] for s in batch if s[2] is not None]), dtype=torch.float32, device=device) # the next state can be None if the game has ended
